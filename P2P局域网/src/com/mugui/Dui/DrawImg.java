...
public class DrawImg extends JPanel {
	...
	private DPanel father = null;

	public DrawImg(DPanel custonimg) {
		father = custonimg;
	...
	private class drawMouseListener implements MouseMotionListener,
			MouseListener {
	...
	private void nextDraw() {
		if(nowimg==null) return;
		if (x2 < x1 || y2 < y1 || x2 > nowimg.getWidth()
				|| y2 > nowimg.getHeight() || x1 < 0 || y1 < 0)
			return;
		if(x1==x2&&y1==y2){
