...
public class WindowControlThread extends Thread {
			...
			while (windowDrawIsTrue) {
				...
				while (windowDrawIsTrue) {
					...
					for (int i = 0; i < map.length && windowDrawIsTrue; i++) {
						...
						bag.setBag_id(UdpBag.USER_WINDOWS_IMG);
	...
	public static boolean start(UdpBag bag2, SocketUserBean tcpSocket2) {
...
class WindowControlBean {
	...
	private SocketUserBean tcpSocket;
	...
	protected SocketUserBean getTcpSocket() {
		return tcpSocket;
	}

	protected void setTcpSocket(SocketUserBean tcpSocket) {
	...
	public WindowControlBean(SocketUserBean tcpSocket2, WindowListenerBean userBean) {
		super();
